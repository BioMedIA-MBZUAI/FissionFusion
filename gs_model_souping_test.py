...
parser.add_argument('--config', type=str, default='./configs/gs_model_souping_test.yaml', metavar='DIR', help='configs')
...
if not os.path.exists(os.path.join("test", "GS", DATASET, MODEL)):
    os.makedirs(os.path.join("test", "GS", DATASET, MODEL))

test_save_path = os.path.join("test", "GS", DATASET, MODEL)

# # Open a text file for logging
log_file_path = os.path.join("test", "GS", DATASET, MODEL, "output_log.txt")
...
for idx, folder in enumerate(sorted(os.listdir(WEIGHT_PATH))):
    ...
    checkpoint = torch.load(os.path.join(WEIGHT_PATH, folder, 'best_checkpoint.pth'), map_location=DEVICE)
